...
test("customer-card practical task is verified from workspace state", async ({ page }, testInfo) => {
  ...
  await page.locator('[data-training-target="nav.sales"]').scrollIntoViewIfNeeded();
  ...
  const beaconStyle = await beacon.evaluate((element) => {
    ...
    const pulseStyle = getComputedStyle(element.querySelector(".driver-hint-pulse") as HTMLElement);
    return {
  ...
  const targetBox = await page.locator('[data-training-target="nav.sales"]').boundingBox();
  const beaconBox = await beacon.boundingBox();
  expect(targetBox).not.toBeNull();
  expect(beaconBox).not.toBeNull();
  expect(Math.abs((beaconBox?.x ?? 0) + (beaconBox?.width ?? 0) / 2 - (targetBox?.x ?? 0) - (targetBox?.width ?? 0))).toBeLessThan(4);
  expect(Math.abs((beaconBox?.y ?? 0) + (beaconBox?.height ?? 0) / 2 - (targetBox?.y ?? 0) - (targetBox?.height ?? 0) / 2)).toBeLessThan(4);
  ...
  await expect(beacon).toHaveCount(1);
  await expect(page.locator('[data-training-target="nav.sales"]')).toBeVisible();
  await expect(page.locator('[data-training-target="sales.counterparties"]')).toBeVisible();
  const nextTargetBox = await page.locator('[data-training-target="sales.counterparties"]').boundingBox();
  const nextBeaconBox = await beacon.boundingBox();
  expect(nextTargetBox).not.toBeNull();
  expect(nextBeaconBox).not.toBeNull();
  expect(Math.abs((nextBeaconBox?.x ?? 0) + (nextBeaconBox?.width ?? 0) / 2 - (nextTargetBox?.x ?? 0) - (nextTargetBox?.width ?? 0))).toBeLessThan(4);
...
test("Driver.js beacon becomes static when reduced motion is requested", async ({ page }) => {
  await page.emulateMedia({ reducedMotion: "reduce" });
  await login(page);
  await page.goto("/learn/customer-card");
  await page.locator('[data-training-target="nav.sales"]').scrollIntoViewIfNeeded();

  const beaconStyle = await page.locator("button.driver-hint.training-driver-hint").evaluate((element) => {
    const style = getComputedStyle(element);
    const pulseStyle = getComputedStyle(element.querySelector(".driver-hint-pulse") as HTMLElement);
    return {
      animationName: pulseStyle.animationName,
...
test("demo mode shows one non-blocking beacon and cleans it up on mode reset", async ({ page }) => {
  await login(page);
  await page.goto("/learn/customer-card");
  ...
  await page.getByRole("button", { name: "Продажи" }).click();
  await page.locator('[data-training-target="sales.counterparties"]').scrollIntoViewIfNeeded();
  await expect(beacon).toHaveCount(1);
  await expect(page.locator('[data-training-target="sales.counterparties"]')).toBeVisible();
