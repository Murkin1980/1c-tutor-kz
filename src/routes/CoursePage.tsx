...
export function CoursePage() {
  ...
  return (
    <div className="page course-page">
      <Link className="back-link" to="/dashboard">← Назад к обучению</Link>
      <div className="course-hero"><div><div className="eyebrow">Курс · {allLessons.length} уроков</div><h1>{course.title}</h1><p>{course.description}</p></div><div className="large-index">01</div></div>
      <div className="module-list">
        {course.modules.map((module, moduleIndex) => (
          <section className="module" key={module.id}>
            <div className="module-heading"><span>Модуль {moduleIndex + 1}</span><div><h2>{module.title.replace(/^\d+\.\s*/, "")}</h2><p>{module.description}</p></div></div>
              ...
              {module.lessons.map((lesson, lessonIndex) => {
                ...
                return unlocked ? (
                  ...
                  <div key={lesson.id} className="lesson-row locked" aria-label={`${lesson.title}, заблокирован`}>
                    <span className="lesson-number"><LockKeyhole size={15} /></span><div><h3>{lesson.title}</h3><span>Завершите предыдущий урок</span></div>
                  </div>
                );
              })}
            </div>
