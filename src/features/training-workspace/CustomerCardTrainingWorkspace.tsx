...
function Target({ id, activeTarget, children, className = "" }: { id: string; activeTarget?: string; children: React.ReactNode; className?: string }) {
...
export function CustomerCardTrainingWorkspace() {
  ...
  const run = (command: CustomerCardCommand) => {
    ...
    update(lessonId, { status: "in_progress", currentStep: state.actionLog.length + 1 });
  };
  ...
  const reset = () => {
    ...
    setHintLevel(0);
    setVerificationVisible(false);
    update(lessonId, { status: "in_progress", currentStep: 0 });
  };
  ...
  const verify = () => {
    ...
    update(lessonId, {
      status: result.passed ? "completed" : "needs_retry",
    ...
    <div className="training-toolbar">
      <div>
        ...
        <div className="training-mode-tabs">
          <button className={mode === "demo" ? "active" : ""} onClick={() => { setMode("demo"); reset(); }}>Показать</button>
          <button className={mode === "guided" ? "active" : ""} onClick={() => { setMode("guided"); reset(); }}>Вести меня</button>
          <button className={mode === "test" ? "active" : ""} onClick={() => { setMode("test"); reset(); }}>Проверить себя</button>
        </div>
      </div>
      <button className="training-reset" onClick={reset}><RotateCcw size={16}/> Начать заново</button>
    </div>
      ...
      <section className="training-app" aria-label="Учебная рабочая область">
        ...
        <div className="training-app-body">
          <nav className="training-nav" aria-label="Разделы учебной программы">
            <button className={state.screen === "home" ? "active" : ""} onClick={() => dispatch({ type: "RESET" })}>Главное</button>
      ...
      <aside className="training-task-panel">
        ...
        {mode !== "test" && guidance && <div className="training-coach">
          ...
          <div className="training-coach-actions">
            <button onClick={() => setHintLevel((level) => Math.min(2, level + 1))}>Подсказка</button>
        ...
        {verificationVisible && <div className={`training-results ${verification.passed ? "success" : "error"}`}>
          <h3>{verification.passed ? "Работа выполнена" : "Нужно исправить"}</h3>
